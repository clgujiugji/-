...
class FallDetection:
    ...
    def __init__(self):
        ...
        self.drive_service = self.create_drive_service()
    ...
    def send_image_message(self, image_url):
        line_bot_api = LineBotApi(LINE_CHANNEL_ACCESS_TOKEN)
    ...
    def detect(self):
        ...
        #cap = cv2.VideoCapture('/Users/qizhihan/Desktop/test5.mp4')
        cap = cv2.VideoCapture(0)
        image_h = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        image_w = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
        #image_h =cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
        #image_w =cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
        frame_num = 0
                ...
                if len(self.joints_list) == ACTION_MODEL_MAX_FRAMES:
                    ...
                    if action_name == 'Fall Down':
                        ...
                        if current_time - self.last_sent_time > 180:
                            self.send_line_notification("転倒を検出しました、画像をcheckしてください")
                            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
                            image_path = f'./fall_data/fall_{timestamp}.jpg'
                            cv2.imwrite(image_path, image)
                            
                         
                        
                        # 上传到 Google Drive 并获取 URL
                            uploaded_file_url = self.upload_to_google_drive(image_path)
                            ...
                            # 使用 LINE Messaging API 发送图片
                            if uploaded_file_url:
                                #self.send_image_message(RECIPIENT_USER_ID, image_url)
                                self.send_image_message(uploaded_file_url)
